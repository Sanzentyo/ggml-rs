...
| save-load-state | In progress | Yes | No | `GenerationSession` + `GenerationCheckpoint` で step-by-step 生成とバイナリ checkpoint を実装。postcard format, model fingerprint 検証, KV cache trimming 対応。11 unit tests passing |
