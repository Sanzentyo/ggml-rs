//! Synthetic GPT-2 batched runner mirroring `main-batched.cpp` behavior.

use clap::{Parser, ValueEnum};
use llama_rs::{LlamaBackend, SyntheticConfig, SyntheticError, run_batched};
...
fn run() -> Result<(), Box<dyn StdError>> {
    ...
    let cli = Cli::parse();
    let report = run_batched(cli.config(), cli.backend.into(), cli.n_parallel)?;
...
#[command(about = "Run synthetic GPT-2 batched execution path", version)]
struct Cli {
    ...
    n_predict: usize,
    #[arg(long = "n-parallel", short = 'p', default_value_t = 4)]
    n_parallel: usize,
