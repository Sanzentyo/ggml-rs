...
use conv::project_and_conv_fused_graph;
use projection::{FusedLinearOutputs, LinearAttentionDims};
use ssm::{SsmScratch, split_and_norm_qk, ssm_recurrence_step};

// Re-exports: keep existing import paths stable for e2e consumers.
pub(super) use conv::causal_depthwise_conv_decode_step;
pub(super) use projection::{LinearProjections, project_linear_inputs};
pub(super) use ssm::LinearDecodeScratch;

// Test-only re-exports for bench_graphs and integration tests.
#[cfg(test)]
pub(super) use conv::{causal_depthwise_conv, causal_depthwise_conv_graph};
...
use super::tensor_ops::{
    head_slice, head_slice_mut, project_sequence_graph, rms_norm_single, rms_norm_single_into,
};
use ggml_rs::Backend;
